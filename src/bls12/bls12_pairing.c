...
void bls12_optate_pairing_projective_7sparse_compress_lazy_montgomery(fp12_t *ANS, efp12_t *P, efp12_t *Q)
{
#ifdef DEBUG_COST_A
    cost tmp;
#endif
    ...
    //Miller's Algo.
    gettimeofday(&tv_start, NULL);
    bls12_miller_algo_for_opt_ate_projective_7sparse_lazy_montgomery(ANS, P, Q);
    gettimeofday(&tv_end, NULL);
    MILLER_OPT_PROJECTIVE_7SPARSE_MONTGOMERY += timedifference_msec(tv_start, tv_end);

#ifdef DEBUG_COST_A
    cost_check(&tmp);
    cost_addition(&MILLER_OPT_PROJECTIVE_7SPARSE_MONTGOMERY_COST, &tmp);
#endif
    ...
    bls12_final_exp_optimal_compress_lazy_montgomery(ANS, ANS);
    gettimeofday(&tv_end, NULL);
    FINALEXP_OPT_PROJECTIVE_7SPARSE_MONTGOMERY += timedifference_msec(tv_start, tv_end);
}
