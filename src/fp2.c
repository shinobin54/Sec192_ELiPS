#include <ELiPS/fp2.h>
//fp2
void fp2_mul_nor_montgomery_fp(fp_t *ANS, fp2_t *A) {
    #if 0
    ...
    mpn_mod_montgomery(ANS->x0, FPLIMB, tmp3, FPLIMB2);
    #elif 1
    static fp_t tmp1,tmp2;
    fp_mulmod_montgomery(&tmp1,&A->x0,&A->x0);
    fp_mulmod_montgomery(&tmp2,&A->x1,&A->x1);
    fp_add(ANS,&tmp1,&tmp2);
    #else
    static mp_limb_t tmp1[FPLIMB2], tmp2[FPLIMB2];
    static mp_limb_t tmp3[FPLIMB];

    // set
    fp_mul_lazy(tmp1, A->x0.x0,A->x0.x0);  // a^2
    fp_mul_lazy(tmp2, A->x1.x0,A->x1.x0);  // b^2
...
void fp2_inv_lazy_montgomery(fp2_t *ANS,fp2_t *A){
    ...
    #else
    static fp_t tmp1_fp;
	fp2_mul_nor_montgomery_fp(&tmp1_fp,A);
    fp_inv_montgomery(&tmp1_fp,&tmp1_fp);
    fp_mulmod_montgomery(&ANS->x0,&A->x0,&tmp1_fp);
    fp_mulmod_montgomery(&tmp1_fp,&A->x1,&tmp1_fp);
    fp_set_neg(&ANS->x1,&tmp1_fp);
    #endif
}
