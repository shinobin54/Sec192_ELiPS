...
void Fp_two_inv_set(){
  ...
  Fp_init(&Fp_two_inv);
  Fp_set_ui(&tmp1_Fp,2);
  Fp_inv(&Fp_two_inv,&tmp1_Fp);
}
void Fp_rshift2(Fp *ANS,Fp *A){
  static Fp tmp1_Fp;
  mpn_rshift(tmp1_Fp.x0,A->x0,FPLIMB,1);
  if(A->x0[0] & 1) Fp_add(ANS,&tmp1_Fp,&Fp_two_inv);
  else Fp_set(ANS,&tmp1_Fp);
}
void Fp_lshift2(Fp *ANS,Fp *A){
  static mp_limb_t buf[FPLIMB];
  mpn_lshift(buf,A->x0,FPLIMB,1);
