...
// Pseudo 8-sparse
void fp2_mul_nor_montgomery(fp2_t *ANS, fp2_t *A) {
    static mp_limb_t tmp1[FPLIMB2], tmp2[FPLIMB2];
    static mp_limb_t tmp3[FPLIMB];

    // set
    fp_sqr_lazy(tmp1, A->x0.x0);  // a^2
    fp_sqr_lazy(tmp2, A->x1.x0);  // b^2

    fp_add_lazy(tmp3, FPLIMB2, tmp1, FPLIMB2, tmp2, FPLIMB2);

    mpn_mod_montgomery(ANS->x0.x0, FPLIMB, tmp3, FPLIMB2);
    mpn_set_ui(ANS->x1.x0, FPLIMB, 0);
}
void fp2_add_mpn_single(fp2_t *ANS, fp2_t *A, mp_limb_t *B) {
    fp_add_mpn(&ANS->x0, &A->x0, B);
    fp_set(&ANS->x1, &A->x1);
}
void fp_sqr_lazy_montgomery(fp_t *ANS, fp_t *A) {
    static mp_limb_t tmp1[FPLIMB2];
    fp_sqr_lazy(tmp1, A->x0);
    mpn_mod_montgomery(ANS->x0, FPLIMB, tmp1, FPLIMB2);
}
...
void Pseudo_8_sparse_mul(fp12_t *ANS, fp12_t *A, fp12_t *B) {
    ...
    fp2_add(&tmp3_fp12.x1.x0, &tmp1_fp2,
            &tmp3_fp2);                        // ans[γ]=tmp1_fp2+tmp3_fp2
    fp2_add(&tmp1_fp2, &tmp2_fp2, &A->x1.x2);  // tmp1_fp2=tmp2_fp2+f5
    fp2_mul(&tmp2_fp2, &A->x0.x2, &B->x1.x0);  // tmp2_fp2=b3*f2
    fp2_add(&tmp3_fp12.x1.x2, &tmp1_fp2,
            &tmp2_fp2);                        // ans[γ^5]=tmp1_fp2+tmp2_fp2
    ...
    fp2_add(&tmp3_fp12.x0.x1, &tmp1_fp2,
            &tmp3_fp2);                        // ans[γ^2]=tmp1_fp2+tmp3_fp2
...
void fp12_7_sparse_mul_lazy_montgomery(fp12_t *C, fp12_t *A, fp12_t *B) {
    fp6_t tmp0_fp6, tmp1_fp6, tmp2_fp6;
    fp2_t v0, v1, v2, v3, v4;

    fp2_mul_lazy_montgomery(&tmp0_fp6.x0, &A->x0.x0, &B->x0.x0);
    //fp2_mul_mpn_montgomery(&tmp0_fp6.x0, &A->x0.x0, B->x0.x0.x0.x0);
    fp2_mul_lazy_montgomery(&tmp0_fp6.x1, &A->x0.x1, &B->x0.x0);
    fp2_mul_lazy_montgomery(&tmp0_fp6.x2, &A->x0.x2, &B->x0.x0);
    // fp2_add(&tmp2_fp6.x0, &B->x0.x0, &B->x1.x0);
    fp2_add_mpn_single(&tmp2_fp6.x0, &B->x0.x0, B->x1.x0.x0.x0);
    fp2_set(&tmp2_fp6.x1, &B->x1.x1);

    // fp6_6_sparse_mul(&tmp1_fp6, &A->x1, &B->x1);
    // fp2_mul_lazy_montgomery(&v0, &A->x1.x0, &B->x1.x0);
    fp2_mul_mpn_montgomery(&v0, &A->x1.x0, B->x1.x0.x0.x0);
    fp2_mul_lazy_montgomery(&v1, &A->x1.x1, &B->x1.x1);
    fp2_add(&v2, &A->x1.x1, &A->x1.x2);
    fp2_mul_lazy_montgomery(&v2, &v2, &B->x1.x1);
    fp2_sub(&v2, &v2, &v1);
    ...
    fp2_add(&v2, &A->x1.x0, &A->x1.x1);
    // fp2_add(&v3, &B->x1.x0, &B->x1.x1);
    fp2_add_mpn_single(&v3, &B->x1.x1, B->x1.x0.x0.x0);
    fp2_mul_lazy_montgomery(&tmp1_fp6.x1, &v2, &v3);
    fp2_sub(&tmp1_fp6.x1, &tmp1_fp6.x1, &v0);
    fp2_sub(&tmp1_fp6.x1, &tmp1_fp6.x1, &v1);
    fp2_add(&v2, &A->x1.x0, &A->x1.x2);
    // fp2_mul_lazy_montgomery(&tmp1_fp6.x2, &v2, &B->x1.x0);
    fp2_mul_mpn_montgomery(&tmp1_fp6.x2, &v2, B->x1.x0.x0.x0);
    fp2_sub(&tmp1_fp6.x2, &tmp1_fp6.x2, &v0);
    fp2_add(&tmp1_fp6.x2, &tmp1_fp6.x2, &v1);
    fp2_set(&tmp1_fp6.x0, &v4);
    // fp6_sparse_mul end
...
void ff_ltt_lazy(fp12_t *f, efp2_t *T, efp_t *P, fp_t *L) {
    ...
    fp2_mul_lazy(&tmp3_fp2, &tmp1_fp2,
                 &tmp2_fp2);  // tmp3_fp2=tmp1_fp2*tmp2_fp2
    ...
    fp2_sub(&tmp5_fp2, &tmp5_fp2, &Tmp_T.y);
    fp2_mul_lazy(&T->y, &tmp3_fp2,
                 &T->x);  // next_T.y=tmp5_fp2-tmp3_fp2*next_T.x
    fp2_sub(&T->y, &tmp5_fp2, &T->y);
...
void ff_ltt_projective_7sparse_lazy_montgomery(fp12_t *f, efp2_projective_t *T, efp_t *P) {
    static fp2_t tmp1_fp2, tmp2_fp2, tmp3_fp2, tmp4_fp2, tmp5_fp2, buf_fp2;
    static fp12_t tmp1_fp12, tmp2_fp12;
    static fp_t tmp0_fp, tmp1_fp;
    efp2_projective_t Tmp_T;
    efp2_projective_init(&Tmp_T);
    efp2_projective_set(&Tmp_T, T);
    ...
    fp_set_neg(&T->x.x1, &T->x.x1);
    fp2_mul_lazy_montgomery(&T->y, &T->y, &T->x);
    fp2_mul_lazy_montgomery(&T->z, &T->z, &T->x);
    fp2_mul_nor_montgomery_fp(&tmp0_fp, &T->x);

    fp12_sqr_lazy_montgomery(&tmp1_fp12, f);

    fp2_sqr_lazy_montgomery(&tmp4_fp2, &T->z);
    fp2_sqr_lazy_montgomery(&tmp1_fp2, &T->y);
    fp2_add(&tmp5_fp2, &tmp4_fp2, &tmp1_fp2);

    fp2_mul_ui(&tmp4_fp2, &tmp4_fp2, 24);

    fp_add(&tmp2_fp2.x0, &tmp4_fp2.x0, &tmp4_fp2.x1);
    fp_sub(&tmp2_fp2.x1, &tmp4_fp2.x1, &tmp4_fp2.x0);

    //fp_sqr_lazy_montgomery(&tmp1_fp, &tmp0_fp);
    fp_mulmod_montgomery(&tmp1_fp, &tmp0_fp, &tmp0_fp);

    fp_hlv(&tmp0_fp,&tmp0_fp);
    fp2_mul_mpn_montgomery(&tmp4_fp2, &T->y, tmp0_fp.x0);
    fp2_dbl(&tmp3_fp2, &tmp2_fp2);
    fp2_add(&tmp3_fp2, &tmp3_fp2, &tmp2_fp2);
    fp2_sub(&T->x, &tmp1_fp2, &tmp3_fp2);
    fp2_mul_lazy_montgomery(&T->x, &T->x, &tmp4_fp2);
    ...
    fp_mulmod_montgomery(&tmp2_fp12.x1.x0.x0, &P->x, &tmp1_fp);
    //fp_add(&tmp1_fp, &tmp2_fp12.x1.x0.x0, &tmp2_fp12.x1.x0.x0);
    //fp_add(&tmp2_fp12.x1.x0.x0, &tmp1_fp, &tmp2_fp12.x1.x0.x0);
    mpn_set_ui(tmp2_fp12.x1.x0.x1.x0, FPLIMB, 0);

    fp_mulmod_montgomery(&tmp2_fp12.x0.x0.x0, &tmp3_fp2.x0, &P->y);
    fp_mulmod_montgomery(&tmp2_fp12.x0.x0.x1, &tmp3_fp2.x1, &P->y);
    //fp2_set_neg(&tmp2_fp12.x0.x0, &tmp2_fp12.x0.x0);

    fp12_7_sparse_mul_lazy_montgomery(f, &tmp1_fp12, &tmp2_fp12);
    //fp12_6_sparse_mul_lazy_montgomery(f, &tmp1_fp12, &tmp2_fp12);
}
void ff_ltt_lazy_montgomery(fp12_t *f, efp2_t *T, efp_t *P, fp_t *L) {
    ...
    fp2_add_lazy(&tmp3_fp2, &tmp2_fp2, &tmp2_fp2);
    fp2_add_lazy(&tmp2_fp2, &tmp3_fp2, &tmp2_fp2);
    fp2_mul_lazy_montgomery(&tmp3_fp2, &tmp1_fp2,
                            &tmp2_fp2);  // tmp3_fp2=tmp1_fp2*tmp2_fp2

    fp2_add(&tmp4_fp2, &Tmp_T.x, &Tmp_T.x);     // tmp4_fp2=2T.x
    fp2_sqr_lazy_montgomery(&T->x, &tmp3_fp2);  // next_T.x=tmp3_fp2^2-tmp4_fp2
    fp2_sub(&T->x, &T->x, &tmp4_fp2);
    fp2_mul_lazy_montgomery(&tmp5_fp2, &tmp3_fp2,
                            &Tmp_T.x);  // tmp5_fp2=tmp3_fp2*T.x-T.y
    fp2_sub(&tmp5_fp2, &tmp5_fp2, &Tmp_T.y);
    fp2_mul_lazy_montgomery(&T->y, &tmp3_fp2,
                            &T->x);  // next_T.y=tmp5_fp2-tmp3_fp2*next_T.x
    fp2_sub(&T->y, &tmp5_fp2, &T->y);
...
void f_ltq_projective_lazy(fp12_t *f, efp2_projective_t *T,
                           efp2_projective_t *Q, efp_t *P) {
    static fp2_t tmp1_fp2, tmp2_fp2, tmp3_fp2, tmp4_fp2, tmp5_fp2;
    static fp12_t tmp1_fp12;
...
void f_ltq_projective_lazy_montgomery(fp12_t *f, efp2_projective_t *T,
                                      efp2_projective_t *Q, efp_t *P) {
    static fp2_t tmp1_fp2, tmp2_fp2, tmp3_fp2, tmp4_fp2, tmp5_fp2;
    static fp12_t tmp1_fp12;
