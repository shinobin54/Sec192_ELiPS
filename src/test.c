...
int test_field(int fp_n,int fp2_n,int fp6_n,int fp12_n,int sqr){
    ...
    float mul_time=0,mul_lazy_time=0,mulmod_montgomery_time=0;
    float inv_time=0,inv_lazy_time=0,inv_montgomery_time=0;
    ...
    fp_t A_fp,B_fp,test1_fp,test2_fp,test3_fp;
    fp2_t A_fp2,B_fp2,test1_fp2,test2_fp2, test3_fp2;
...
printf("------------------------------------------------------------------------------------\n");
    printf("fp_mul test\n");
mul_time=0,mul_lazy_time=0,mulmod_montgomery_time=0;
n=1000;
for(i=0;i<fp_n;i++){
    fp_set_random(&A_fp,state);
    fp_set_random(&B_fp,state);
    ...
    fp_to_montgomery(&A_fp,&A_fp);
    fp_to_montgomery(&B_fp,&B_fp);
    gettimeofday(&tv_A,NULL);
    for(j=0;j<n;j++)fp_mulmod_montgomery(&test3_fp,&A_fp,&B_fp);
    gettimeofday(&tv_B,NULL);
    mulmod_montgomery_time+=timedifference_msec(tv_A,tv_B)/n;
    ...
    printf("fp mulmod monty: %.6f[ms]\n",mulmod_montgomery_time/fp_n);
...
for(i=0;i<fp2_n;i++){
    ...
    gettimeofday(&tv_A,NULL);
    for(j=0;j<n;j++)fp2_mul_lazy_montgomery(&test3_fp2,&A_fp2,&B_fp2);
    gettimeofday(&tv_B,NULL);
    mulmod_montgomery_time+=timedifference_msec(tv_A,tv_B)/n;
    ...
    printf("fp2 mulmod montgomery. : %.6f[ms]\n",mulmod_montgomery_time/fp2_n);
    #ifdef DEBUG_COST_A
...
printf("------------------------------------------------------------------------------------\n");
    printf("fp2_inv test\n");
inv_time=0,inv_lazy_time=0,inv_montgomery_time=0;
n=1000;
for(i=0;i<fp2_n;i++){
    fp2_set_random(&A_fp2,state);
    fp2_set_random(&B_fp2,state);
    ...
    fp2_to_montgomery(&A_fp2,&A_fp2);
    gettimeofday(&tv_A,NULL);
    for(j=0;j<n;j++)fp2_inv_lazy_montgomery(&test3_fp2,&A_fp2);
    gettimeofday(&tv_B,NULL);
    inv_montgomery_time+=timedifference_msec(tv_A,tv_B)/n;

}
    printf("fp2 inv.      : %.6f[ms]\n",inv_time/fp2_n);
    printf("fp2 inv_lazy. : %.6f[ms]\n",inv_lazy_time/fp2_n);
    printf("fp2 inv_montgomery. : %.6f[ms]\n",inv_montgomery_time/fp2_n);
